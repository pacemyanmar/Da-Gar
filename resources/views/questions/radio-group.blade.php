...
<table class="table table-responsive" >
	...
	<tr>
		...
		@endif
		@if($radio->other)
		{!! Form::text("result[".$radio->inputid."]", (isset($results) && !empty($results['section'.$section->sort]) && $radio->value == $results['section'.$section->sort]->{$radio->inputid})?$results['section'.$section->sort]->{$radio->inputid}:null, ['class' => $radio->className.' form-control input-sm', 'autocomplete' => 'off', 'id' => $radio->id.'other', 'style' => 'width:80%']) !!}
			@push('document-ready')
