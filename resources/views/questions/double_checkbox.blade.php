<div class="form-group">
    ...
    $element->value, (isset($double_results) && !empty($double_results['section'.$section->sort]) &&
    $element->value == $double_results['section'.$section->sort]->{$element->inputid}),
    ['data-class'=>$element->inputid, 'data-origin'=>(isset($results)  && !empty($results['section'.$section->sort]))? $results['section'.$section->sort]->{$element->inputid}:null,'class' => 'magic-checkbox '.$element->className.' '.$sectionClass, 'id' => $element->id, 'autocomplete' => 'off']) !!}
